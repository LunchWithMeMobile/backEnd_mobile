...
router.post('/forgotpasswordEmailVerificationMobile',(req,res,next)=>{
  User.find({email: req.body.email})
    .then(users => {
      if(users.length==0){
        return res.status(400).json({'msg': 'user not found'});
...
router.post('/Mregister', (req, res, next) => {
  let response = {success: false};
  ...
  if (!(req.body.password == req.body.confirmPass)) {
    ...
    User.addUser(newUser, (err, user) => {
      if (err) {
       ...
       emailhandler.mailhandleremailconfirm2(user.email,user.username,'1212', verifiedToken);
        console.log("[%s] registered successfuly", user.username);
        res.json(response);
      }
    });
...
// to activate mobile users' accounts
router.get('/Mactive', function(req, res) {
  const token = req.query.token;
//from findone() the relevent document(tuple) is returned.If there is no such relevent document then it will return null.
...
// console.log("email usr - "+req.para
//User.updateOne({email:req.params.email},{active: true},{upsert: true}).then(doc=>{
//   console.log("succss - "+JSON.stringify(doc))
//   return res.status(201).json(doc);
// })  
